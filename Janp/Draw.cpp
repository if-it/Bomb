...
{
	if (dis)DrawRotaGraphFast3F(pos.x - sc.x + shake.x, pos.y - sc.y + shake.y, texPos.x, texPos.y, up.x, up.y, PI / 180 * ang, tex, flg, turn);
}
...
void Draw::DrawRotaTex(const GameObject& chara, const int& tex, bool flg, const Vector2& shake, const Vector2& sc, bool center)
{
	if (!center)
	{
		if (chara.game.dis)DrawRotaGraphFast3F(chara.game.allVec.pos.x + chara.game.size.x / 2 - sc.x + shake.x,
	...
	{
		if (chara.game.dis)DrawRotaGraphFast3F(chara.game.allVec.pos.x - sc.x + shake.x, chara.game.allVec.pos.y - sc.y + shake.y,
			chara.game.size.x / 2, chara.game.size.y / 2, chara.game.scale.x, chara.game.scale.y, PI / 180 * chara.game.rota, tex, flg, chara.game.lr);
	}
